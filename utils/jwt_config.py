"""
JWT Configuration for Main Project
...
class JWTConfig:
    ...
    def __str__(self) -> str:
        ...
        return f"JWTConfig({config_dict})"
...
def create_jwt_config(secret_key: str, config_file: str) -> JWTConfig:
    ...
    Raises:
        ValueError: 當配置無效時拋出異常
        FileNotFoundError: 當配置檔案不存在時拋出異常
    """
    config = JWTConfig(secret_key=secret_key, config_file=config_file)
    
    if not config.validate():
        raise ValueError("JWT 配置驗證失敗")
